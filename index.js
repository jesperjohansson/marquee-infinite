...
class MarqueeInfinite {
   ...
   */
  constructor(
    selectorOrElement = ".js-marquee-infinite",
   ...
   */
  cacheInitialItems() {
    this.initialItemElements = this.containerDirectChildren.map(child =>
  ...
  removeContainerDirectChildren() {
    this.containerDirectChildren.forEach(child =>
  ...
  getAllItemsWidth() {
    return (
      this.items.length *
      (this.initialItemsWidth / this.initialItemElements.length)
    );
  }
  ...
  fillWithItems() {
    ...
    for (
      ...
      i += this.initialItemElements.length
    ) {
      ...
      if (i >= this.options.maxItems - 1 && !this.getIsFilled()) {
        console.warn(
          `Reached the max items limit of ${
  ...
  getAnimationDuration() {
    const itemsWidth = this.items.reduce(
      (sum, item) => sum + getWidth(item),
      0
    );
   ...
   */
  events() {
    const resizeDebouncer = debounce(this.handleResize, 1000);
